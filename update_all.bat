...
$activePackages = @('fenix-web-server', 'fenix-web-server-pre', 'thunderbird-mozilla', 'thunderbird-nightly', 'github-desktop-pre', 'nicepage', 'cloudflare-warp-pre')
$report = @()
$globalUpdated = $false
...
foreach ($packageName in $activePackages) {
    ...
    switch ($packageName) {
        ...
        'fenix-web-server-pre'  { $cf = 32; function global:au_SearchReplace { @{ ".\tools\chocolateyinstall.ps1" = @{ '(?i)(\$url\s*=\s*)(''.*'')' = "`$1'https://github.com/coreybutler/fenix/releases/download/$($Latest.OriginalVersion)/Fenix.Setup.$($Latest.OriginalVersion).exe'" } } }; function global:au_GetLatest { $req = Invoke-RestMethod -Uri 'https://api.github.com/repos/coreybutler/fenix/releases'; $latest = $req | Where-Object { $_.prerelease -eq $true } | Select-Object -First 1; $urlV = $latest.tag_name.Replace('v', ''); return @{ Version = ($urlV -replace '-rc\.', '-rc'); OriginalVersion = $urlV; URL32 = ($latest.assets | Where-Object name -like "Fenix.Setup*.exe").browser_download_url } } }
        'thunderbird-mozilla'   { function global:au_SearchReplace { @{ ".\tools\chocolateyinstall.ps1" = @{ '(?i)(\$version\s*=\s*)(''.*'')' = "`$1'$($Latest.OriginalVersion)'" } } }; function global:au_GetLatest { $req = Invoke-RestMethod -Uri 'https://product-details.mozilla.org/1.0/thunderbird_versions.json'; $rv = $req.LATEST_THUNDERBIRD_DEVEL_VERSION; $csUrl = "https://download-installer.cdn.mozilla.net/pub/thunderbird/releases/$rv/SHA256SUMS"; try { Invoke-WebRequest -Uri $csUrl -OutFile ".\tools\checksums.txt" -ErrorAction Stop } catch { Write-Warning "No se pudo descargar checksums para Thunderbird Beta" }; return @{ Version = ($rv -replace 'b', '-beta'); OriginalVersion = $rv; URL64 = $csUrl } } }
        'thunderbird-nightly'   { function global:au_SearchReplace { @{ ".\tools\chocolateyinstall.ps1" = @{ '(?i)(\$version\s*=\s*)(''.*'')' = "`$1'$($Latest.OriginalVersion)'" } } }; function global:au_GetLatest { $req = Invoke-RestMethod -Uri 'https://product-details.mozilla.org/1.0/thunderbird_versions.json'; $rv = $req.LATEST_THUNDERBIRD_NIGHTLY_VERSION; $baseUrl = "https://ftp.mozilla.org/pub/thunderbird/nightly/latest-comm-central-l10n";
                $csUrl = "$baseUrl/thunderbird-$rv.en-US.win64.checksums"; try { Invoke-WebRequest -Uri $csUrl -OutFile ".\tools\checksums.txt" -ErrorAction Stop } catch { Write-Warning "No se pudo descargar checksums para Thunderbird Nightly" }; return @{ Version = ($rv -replace 'a', '-nightly'); OriginalVersion = $rv; URL64 = $csUrl } } }
        'github-desktop-pre'    { $cf = 64; function global:au_SearchReplace { @{ ".\tools\chocolateyinstall.ps1" = @{ '(?i)(\$url\s*=\s*)(''.*'')' = "`$1'$($Latest.URL64)'" } } }; function global:au_GetLatest { $req = Invoke-RestMethod -Uri 'https://central.github.com/api/deployments/desktop/desktop/latest?version=beta&os=windows'; return @{ Version = ($req.version -replace '-beta', '-beta'); URL64 = ($req.url -replace "GitHubDesktop-x64.zip", "GitHubDesktopSetup-x64.exe") } } }
